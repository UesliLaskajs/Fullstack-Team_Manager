            ...
            <div className='list_player-container'>
                ...
                {players.map((item, index) => (
                    <div key={index} className='row_values'>
                        ...
                        <button
                            className={` ${item.game1_status === 'Playing' ? 'greenButton' : null}`}
                            onClick={() => { updatePlayer(item._id, 'Playing'); setStatus('Playing'); }}
                        >
                            Playing
                        </button>
                        <button
                            className={` ${item.game1_status === 'Not Playing' ? 'redButton' : null}`}
                            onClick={() => { updatePlayer(item._id, 'Not Playing'); setStatus('Not Playing'); }}
                        >
                            Not Playing
                        </button>
                        <button
                            className={` ${item.game1_status === 'Undecided' ? 'yellowButton' : null}`}
