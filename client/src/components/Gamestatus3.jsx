            ...
            <h1>Player Status Game 3</h1>
                {players.map((item, index) => (
                    ...
                    <div key={index} className='row_values'>
                        ...
                        <button
                            className={` ${item.game3_status === 'Playing' ? 'greenButton' : null}`}
                            onClick={() => { updatePlayer(item._id, 'Playing'); setStatus('Playing'); }}
                        >
                            Playing
                        </button>
                        <button
                            className={` ${item.game3_status === 'Not Playing' ? 'redButton' : null}`}
                            onClick={() => { updatePlayer(item._id, 'Not Playing'); setStatus('Not Playing'); }}
                        >
                            Not Playing
                        </button>
                        <button
                            className={` ${item.game3_status === 'Undecided' ? 'yellowButton' : null}`}
